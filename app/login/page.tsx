    ...
    const handleEmailLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (err.code === 'auth/invalid-credential') {
                setError(t.auth.errors.wrongPassword); // Or generic credential error
            } else if (err.code === 'auth/user-not-found') {
                setError(t.auth.errors.userNotFound);
            } else if (err.code === 'auth/operation-not-allowed') {
                setError('Login method disabled.');
            } else {
    ...
    const handlePhoneLogin = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (err.code === 'auth/operation-not-allowed') {
                ...
                setError('Phone login unavailable. Please use Email or Google login.');
        ...
        <main className="flex-1 flex flex-col items-center justify-center min-h-screen bg-slate-50 dark:bg-[#111418] px-4">
            <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 border border-slate-100 dark:border-slate-700">
                <div className="text-center mb-8">
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-2">{t.auth.signIn}</h1>
